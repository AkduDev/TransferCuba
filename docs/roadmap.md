...
4. Fotos: **solo al abrir ficha** — `loading="lazy"`, WebP (next/image nativo) ✅ · Almacenamiento en Cloudinary (upload unsigned, `POST /api/businesses/[id]/images` para persistir URL en `business_images`) ✅
...
- [x] **Sprint 5 (MVT — Fase 4)**: vector tiles habilitados. `get_businesses_mvt(z,x,y)` en `db/schema.sql` (PostGIS puro: `ST_AsMVT` + `ST_TileEnvelope`, sin extensiones extra; bbox filtrado en 4326 por el tipo `geography` de `geom`, proyección a 3857 solo dentro de `ST_AsMVTGeom`) + endpoint `GET /api/tiles/[z]/[x]/[y]` → `queryBusinessesMvt` en `lib/db.ts` (pool + circuit breaker; dev sin BD → tile vacío; validación enteros/z≤22/x,y<2**z; `Content-Type: application/vnd.mapbox-vector-tile`; `Cache-Control: s-maxage=300, stale-while-revalidate=3600`). Payload ligero: id/name/category/icons/province/municipality/flags/rating, solo `status='active'`. Frontend migrada a MVT (source vectorial `businesses-mvt` en `MapLibreMap.tsx`, capas circle + symbol; selección vía overlay GeoJSON pequeño). GeoJSON+clusters eliminados.
