...
- [x] **Sprint 5 (MVT — Fase 4)**: vector tiles habilitados. `get_businesses_mvt(z,x,y)` en `db/schema.sql` (PostGIS puro: `ST_AsMVT` + `ST_TileEnvelope`, sin extensiones extra; bbox filtrado en 4326 por el tipo `geography` de `geom`, proyección a 3857 solo dentro de `ST_AsMVTGeom`) + endpoint `GET /api/tiles/[z]/[x]/[y]` → `queryBusinessesMvt` en `lib/db.ts` (pool + circuit breaker; dev sin BD → tile vacío; validación enteros/z≤22/x,y<2**z; `Content-Type: application/vnd.mapbox-vector-tile`; `Cache-Control: s-maxage=300, stale-while-revalidate=3600`). Payload ligero: id/name/category/icons/province/municipality/flags/rating, solo `status='active'`. Verificado: z12/1110/1777 → 1040 B con capa `businesses` y datos reales, z14/4442/7109 → 285 B, tile vacío 0 B/200, z23/coords inválidas → 400; tsc 0, build OK (nueva route en el tree), smoke4 E2E en verde. La frontend sigue con GeoJSON+clusters (Sprint 9); la fuente vector queda lista para el cutover.
